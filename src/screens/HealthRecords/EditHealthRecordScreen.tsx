...
import HealthRecordService, { HealthRecord, Veterinarian, HealthRecordFormData } from '../../services/HealthRecordService';
...
export default function EditHealthRecordScreen() {
  ...
  const route = useRoute<RouteProp<RootStackParamList, 'EditHealthRecord'>>();
  ...
  const fetchRecord = async () => {
    try {
      ...
      Alert.alert('Error', 'Failed to load health record for editing');
      navigation.goBack();
    } finally {
      setLoading(false);
    }
  ...
  const takePhoto = async () => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        const newImagesList = [...newImages, result.assets[0].uri];
  ...
  const deleteRemovedImages = async () => {
    ...
    try {
      for (const imageUrl of deletedImages) {
        ...
        await HealthRecordService.deleteHealthRecordImage(recordId, imageId);
      }
    } catch (error) {
  ...
  const saveHealthRecord = async () => {
    ...
    try {
      ...
      // Prepare new images if we have any
      ...
      await HealthRecordService.updateHealthRecord(recordId, recordData);
  ...
  // Loading state
  if (loading || loadingAnimals || loadingVets) {
    return (
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Edit Health Record</Text>
          ...
          <View style={styles.formGroup}>
            ...
            <View style={styles.imagesContainer}>
              {newImages.length > 0 ? (
                ...
                <Text style={styles.noImagesText}>No new images attached</Text>
              )}
            </View>
          ...
          <View style={styles.submitButtonContainer}>
            ...
            >
              {savingRecord ? (
                ...
                <Text style={styles.submitButtonText}>Update Health Record</Text>
              )}
            </TouchableOpacity>
          </View>
        </ScrollView>
