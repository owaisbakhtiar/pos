...
import AnimalService, { AnimalCreateRequest } from '../../services/AnimalService';
...
export default function AddAnimalScreen() {
  ...
  // Fetch shed locations when component mounts
  useEffect(() => {
    fetchShedLocations();
  }, []);

  // Fetch shed locations from API
  const fetchShedLocations = async () => {
    setIsLoading(true);
    try {
      const response = await api.get('/v1/livestock/shedLocation');
      if (response.data.success) {
        setShedLocations(response.data.data.data || []);
      } else {
        Alert.alert('Error', 'Failed to load shed locations');
      }
    } catch (error) {
      console.error('Error fetching shed locations:', error);
      Alert.alert('Error', 'Failed to load shed locations. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleInputChange = (field: keyof AnimalCreateRequest, value: string) => {
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        ...
        <View style={styles.buttonContainer}>
          ...
          />
          <Button
            title={isSaving ? "Saving..." : "Save Animal"}
            onPress={handleSave}
